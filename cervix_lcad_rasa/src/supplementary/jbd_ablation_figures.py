...
def fig_modality_barplot(project: Path, out_dir: Path) -> None:
    ...
    sns.barplot(data=df, y="label", x="auc", palette=colors, ax=axes[0], orient="h", legend=False)
    axes[0].axvline(ref, color=C6, ls="--", lw=1)
    axes[0].set_xlabel("AUROC (test, threshold-free)")
    axes[0].set_ylabel("")
    axes[0].set_title("Modality ablation — AUROC")
    sns.barplot(data=df, y="label", x="f1", palette=PALETTE_MAIN[: len(df)], ax=axes[1], orient="h", legend=False)
    axes[1].set_xlabel("F1 (validation max-F1 threshold on test)")
    axes[1].set_title("Modality ablation — F1")
    fig.suptitle("Supplementary: input modality subsets (n = 288 test)", y=1.02, fontsize=13)
    fig.tight_layout()
...
def fig_rasa_delta(project: Path, out_dir: Path) -> None:
    ...
    sns.barplot(data=df, x="delta_auc", y="label", palette=colors, ax=axes[0], orient="h", legend=False)
    ...
    sns.barplot(data=df, x="delta_f1", y="label", palette=colors2, ax=axes[1], orient="h", legend=False)
...
def fig_rasa_auc_f1_scatter(project: Path, out_dir: Path) -> None:
    ...
    sns.scatterplot(data=df, x="auc", y="f1", hue="experiment_id", s=160, palette=PALETTE_MAIN, ax=ax, edgecolor=C7, linewidth=0.8)
...
def fig_qc_barplot(project: Path, out_dir: Path) -> None:
    ...
    g = sns.catplot(data=melt, x="score", y="label", col="metric", kind="bar", palette=[C0, C4], height=5, aspect=0.85, sharex=False)
...
def write_ablation_figure_index(out_dir: Path) -> None:
    ...
    lines = [
        ...
        "Font: Times New Roman (serif); mathtext: STIX.\n\n",
    ]
